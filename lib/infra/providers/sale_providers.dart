...
class SaleNotifier extends StateNotifier<List<SaleModel>> {
  ...
  Future<List<SaleModel>> refreshSales(UserModel userModel) async {
    ...
    state = await repository.get('http://zuplae.vps-kinghost.net:8083/api/Sale/User/${userModel.id}');
    return state;
  }

  Future addSalesContainer(SaleModel saleModel) async {
    SaleRepository repository = SaleRepository();
    await repository.post('http://zuplae.vps-kinghost.net:8083/api/Sale', saleModel);
    state = await repository.get('http://zuplae.vps-kinghost.net:8083/api/Sale/User/${saleModel.userId}');

    ProductRepository productRepository = ProductRepository();
    ProductModel product = await productRepository.getById('http://zuplae.vps-kinghost.net:8083/api/Product', saleModel.productId);
    product.quantity -= saleModel.quantity;
    await productRepository.put('http://zuplae.vps-kinghost.net:8083/api/Product', product);
  }
}
