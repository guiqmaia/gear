...
class CategoryNotifier extends StateNotifier<List<CategoryModel>> {
  ...
  Future<List<CategoryModel>> getAllCategories(UserModel userModel) async {
    ...
    state = await repository.get('http://zuplae.vps-kinghost.net:8083/api/Category/User/${userModel.id}');
    return state;
  }

  Future<void> getByIdCategory(CategoryModel category) async {
    CategoryRepository repository = CategoryRepository();
    await repository.getById('http://zuplae.vps-kinghost.net:8083/api/Category', category.id!);
  }

  Future<void> addCategory(CategoryModel category) async {
    CategoryRepository repository = CategoryRepository();
    await repository.post('http://zuplae.vps-kinghost.net:8083/api/Category', category);
    state = await repository.get('http://zuplae.vps-kinghost.net:8083/api/Category/User/${category.userId}');
  }
}
...
class ProductNotifier extends StateNotifier<List<ProductModel>> {
  ...
  Future<void> addProduct(ProductModel product, CategoryModel model) async {
    ProductRepository repository = ProductRepository();
    await repository.post('http://zuplae.vps-kinghost.net:8083/api/Product', product);
    state = await repository.getProductByCategory(model.id!);
  }
}
